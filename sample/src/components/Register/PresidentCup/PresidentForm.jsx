...
function PresidentForm() {
    ...
    const handleFormSubmit = async (formErrors) => {
        if(Object.keys(formErrors).length === 0 && currentStep == steps.length){
            ...
            try{
                setLoading(true);
                axios.post("/api/emmaapi", formData).
                then(res => {
                    setTimeout(() => {
                       router.push({
                            pathname: "/payment",
    ...
    return (
        <RegisterContext.Provider value={presidentContext}>
            ...
            <div data-aos="zoom-in" data-aos-duration="3000" className={`lg:max-w-[50%] lg:mx-auto my-4 rounded-sm p-8 `}>
                ...
                <AuthHeader miniTitle={"Registration Form"}/>
                <Stepper 
                    steps={steps}
                    currentStep={currentStep}
                    />

                <div className="container mx-auto auth-bg rounded-md py-4 px-6 lg:px-16 shadow-xl shadow-gray-500 border-8 border-solid !border-secondary">
