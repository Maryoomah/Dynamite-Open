...
function Field({
  ...
  const labelClass =
    "text-[11px] font-black uppercase tracking-widest text-white/50 ml-2";
  ...
  if (type === "select") {
    return (
      <div className={className + " space-y-2"}>
        ...
        {!!error && (
          <p className="ml-2 text-[10px] font-bold text-red-300 uppercase tracking-wider">
            {error.message}
          </p>
        )}
      </div>
    ...
    <div className="relative p-1 rounded-4xl sm:rounded-[2.8rem] bg-linear-to-tr from-green-500/20 via-white/5 to-yellow-500/20 shadow-2xl transition-all duration-500 hover:shadow-green-500/10">
      <div className="relative bg-[#0d3b28]/95 backdrop-blur-3xl rounded-4xl sm:rounded-[2.5rem] p-6 sm:p-12 border border-white/10">
        <div className="mb-8 sm:mb-10 space-y-2">
          ...
          {subtitle && (
            <p className="mt-4 text-green-100/60 font-medium text-xs sm:text-sm leading-relaxed max-w-xl">
              {subtitle}
            </p>
          )}
        </div>
  ...
  return (
    <div className="rounded-3xl sm:rounded-4xl bg-white/5 border border-white/10 p-5 sm:p-8 space-y-6">
      <div className="flex items-center justify-between gap-3 border-b border-white/5 pb-4">
        <h4 className="font-black text-[10px] sm:text-xs uppercase tracking-[0.2em] text-yellow-400">
          {heading}
        </h4>
...
export default function SchoolRegistration() {
  ...
  const scrabblePlayers = [
    {
      key: "scrabbleMale1",
      heading: "Scrabble — Male Player 1",
      gender: "male",
    },
    {
      key: "scrabbleMale2",
      heading: "Scrabble — Male Player 2",
      gender: "male",
    },
    {
      key: "scrabbleFemale1",
      heading: "Scrabble — Female Player 1",
      gender: "female",
    },
    {
      key: "scrabbleFemale2",
      heading: "Scrabble — Female Player 2",
      gender: "female",
    },
  ];
  ...
  const beePlayers = [
    {
      key: "beeMale",
      heading: "Spelling Bee — Male Participant",
      gender: "male",
    },
    {
      key: "beeFemale",
      heading: "Spelling Bee — Female Participant",
      gender: "female",
    },
  ];
  ...
  return (
    <section className="min-h-screen py-10 sm:py-16 bg-[#0a2e1f] relative overflow-hidden">
      <SEO
        ...
        description="Register your school team for the Dynamite Opens inter-school competition."
      />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 relative z-10 w-full">
        <div className="text-center sm:text-left space-y-4 mb-10 sm:mb-16">
          ...
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-white uppercase tracking-tighter leading-none">
            Team{" "}
            <span className="text-transparent bg-clip-text bg-linear-to-r from-yellow-500 to-amber-400">
              Registration
            </span>
          </h1>
          <p className="text-green-100/60 font-medium text-base sm:text-lg leading-relaxed max-w-xl">
            Empowering the next generation of Scrabble masters. Register your
            school's finest players today.
          </p>
        </div>

        <form
          onSubmit={handleSubmit(onSubmit)}
          className="space-y-10 sm:y-16 group/form"
        >
          <Card
  ...
  validation={{
    required: "Required",
    validate: (v) => v <= "2018-12-31" || "DOB must be 2018 or earlier",
  }}
  error={errors[`${p.key}Dob`]}
/>


  {/* Visible gender display (not editable) */}
  <div className="space-y-2">
    <label className="text-[11px] font-black uppercase tracking-widest text-white/50 ml-2">
      Gender
    </label>

    <div className="w-full rounded-2xl bg-white/10 text-white border border-white/20 px-5 py-4 font-black uppercase tracking-widest text-xs flex items-center justify-between">
      <span className="text-white">{p.gender}</span>
    </div>

    {/* Hidden field so it still submits */}
    <input type="hidden" value={p.gender} {...register(`${p.key}Gender`)} />
  </div>
</div>
                    <Field
    ...
    <input type="hidden" value={p.gender} {...register(`${p.key}Gender`)} />
                  ...
                  <div className="space-y-3">
                    <label className="flex items-center gap-4 cursor-pointer group/agree">
                      <div className="relative">
                        ...
                        />
                        <div
                          className={`h-6 w-6 sm:h-7 sm:w-7 rounded-lg border-2 flex items-center justify-center transition-all ${errors.agreed ? "border-red-400 bg-red-400/10" : "border-white/20 peer-checked:bg-yellow-500 peer-checked:border-yellow-500"} peer-checked:[&_svg]:scale-100`}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className="h-4 w-4 sm:h-5 sm:w-5 text-green-950 scale-0 transition-transform"
                            viewBox="0 0 20 20"
                            fill="currentColor"
                          >
                            <path
                    ...
                    {errors.agreed && (
                      <p className="ml-12 text-[10px] font-bold text-red-300 uppercase tracking-wider">
        ...
        <div className="mt-12 sm:mt-16 flex items-center justify-center gap-4 py-8 border-t border-white/5">
          <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.4em]">
            Dynamite Opens Excellence 2026
          </p>
        </div>
      </div>
    </section>
